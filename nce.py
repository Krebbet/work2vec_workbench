...
class Model(Net):
  ...
  def __init__(self, model_param,specific_param,train_model = True):
    ...
    self.negative_samples = specific_param['negative_samples']
  ...
  def inference(self,train_model):
    ...
    self.embed = tf.nn.embedding_lookup(params = self.embeddings, 
  ...
  def loss(self):
    ...
    # grab the linear weights from our embedding to 
    # create the nce loss 
    ...
    nce_loss = tf.nn.nce_loss(weights=tf.transpose(W),    # [embedding, vocab]
                       ...
                       labels=self.context, # [batch]
                       inputs=self.embed,   # [batch,embedding size]
                       num_sampled=self.negative_samples,
                       num_classes=self.vocabulary_size)
    
    L_embed = tf.reduce_mean(nce_loss)
